...
public class CommandThiefSpawn extends CommandThiefBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (world == null || world.isRaining()) {
            notifyCommandListener(sender, this, ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".badconditions");
            LoggerUtils.info(TextUtils.translate(ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".badconditions"), true);
            return;
        }

        if (world.getDifficulty() == EnumDifficulty.PEACEFUL && !ModConfig.thief.thiefSpawnsWhenPeaceful) {
            notifyCommandListener(sender, this, ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".peaceful");
            LoggerUtils.info(TextUtils.translate(ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".peaceful"), true);
            return;
        }
        ...
        if (spawnPosition == null) {
            notifyCommandListener(sender, this, ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".noposition");
            LoggerUtils.info(TextUtils.translate(ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".noposition"), true);
            return;
        }

        List<EntityThief> entityList = world.getEntitiesWithinAABB(EntityThief.class, village.getAABB().grow(Village.VILLAGE_SIZE));
        if (entityList.size() > 0) {
            notifyCommandListener(sender, this, ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".exists");
            LoggerUtils.info(TextUtils.translate(ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".exists"), true);
            return;
        }

        // attempt to spawn the thief
        if (!TektopiaUtils.trySpawnEntity(world, spawnPosition, (World w) -> new EntityThief(w, level))) {
            notifyCommandListener(sender, this, ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".failed");
            LoggerUtils.info(TextUtils.translate(ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".failed"), true);
            return;
        }

        notifyCommandListener(sender, this, ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".success", TektopiaUtils.formatBlockPos(spawnPosition));
        LoggerUtils.info(TextUtils.translate(ThiefCommands.COMMAND_PREFIX + COMMAND_NAME + ".success", TektopiaUtils.formatBlockPos(spawnPosition)), true);
    }
