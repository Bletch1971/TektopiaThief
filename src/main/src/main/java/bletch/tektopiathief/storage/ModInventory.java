...
public class ModInventory extends InventoryBasic {
	...
	private EntityThief owner;

	public ModInventory(EntityThief owner, String title, boolean customName, int slotCount) {
	...
	private List<ItemStack> findItems(Function<ItemStack, Integer> func, int itemCount, boolean remove) {
		...
		List<Tuple<ItemStack, Integer>> matchedItems = new ArrayList<Tuple<ItemStack, Integer>>();

		for (int i = 0; i < this.getSizeInventory(); ++i) {
			ItemStack itemStack = this.getStackInSlot(i);

			if (net.tangotek.tektopia.ModItems.canVillagerSee(itemStack) && func.apply(itemStack) > 0) {
				matchedItems.add(new Tuple<ItemStack, Integer>(itemStack, i));
			}
		}

		matchedItems.sort(Comparator.comparing(pair -> func.apply((ItemStack)pair.getFirst())));

		for (int i = matchedItems.size() - 1; i >= 0 && needed > 0; --i) {

			Tuple<ItemStack, Integer> pair2 = matchedItems.get(i);
			ItemStack itemStack2 = (ItemStack)pair2.getFirst();
			int slot = (int)pair2.getSecond();
	...
	public List<ItemStack> getItems(Predicate<ItemStack> pred, int itemCount) {
		return this.findItems(p -> Integer.valueOf(pred.test(p) ? 1 : 0), itemCount, false);
	}
