...
public class EntityAIUseGate extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (path != null && !path.isFinished() && pathNavigate.getEnterDoors()) {
			for(int i = 0; i < Math.min(path.getCurrentPathIndex() + 2, path.getCurrentPathLength()); ++i) {
				...
				if (this.entity.getDistanceSq((double)this.gatePosition.getX(), this.entity.posY, (double)this.gatePosition.getZ()) <= 2.25D) {
	...
	public void startExecuting() {
		LoggerUtils.info("EntityAIUseGate - startExecuting called", true);
	...
	public void resetTask() {
		LoggerUtils.info("EntityAIUseGate - resetTask called", true);
	...
	protected void toggleGate(boolean open) {
		...
		if (this.getBlockGate(gatePosition) != null && ((Boolean)iblockstate.getValue(BlockFenceGate.OPEN)).booleanValue() != open) {
			iblockstate = iblockstate.withProperty(BlockFenceGate.OPEN, open);
			this.entity.world.setBlockState(gatePosition, iblockstate, 10);
			this.entity.world.playEvent((EntityPlayer)null, ((Boolean)iblockstate.getValue(BlockFenceGate.OPEN)).booleanValue() ? 1008 : 1014, this.gatePosition, 0);
		}
	}
}
